        ...
        stage("connect to deploy server"){
            ...
            steps {
                ...
                script {
                    ...
                    ssh -i $SSH_CRED -t -o StrictHostKeyChecking=no ubuntu@ec2-54-197-15-121.compute-1.amazonaws.com << EOF
                    curl ifconfig.co/ip
                    df -h
                    ...
                    sudo mkdir app
                    cd app
                    sudo git clone https://github.com/Delali97/nest_js_app.git .
                    sudo npm install -y
